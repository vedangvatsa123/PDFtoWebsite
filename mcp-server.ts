 ...
 *       "cvinbio": {
 *         "command": "npx",
 *         "args": ["tsx", "/path/to/PDFtoWebsite/mcp-server.ts"],
 *         "env": {
...
function formatProfile(p: any): string {
  ...
  const location =
    (Array.isArray(p.links) &&
      p.links.find((l: any) => l.type === "location")?.value) ||
    "";

  let out = `## ${name}\n`;
  out += `Profile: ${SITE}/${slug}\n`;
  if (location) out += `Location: ${location}\n`;
  if (skills.length > 0) out += `Skills: ${skills.join(", ")}\n`;
  if (about) out += `\nSummary: ${about}\n`;
  ...
  if (experience.length > 0) {
    ...
    for (const job of experience.slice(0, 5)) {
      ...
      if (job.location) out += ` [${job.location}]`;
      out += `\n`;
    }
  }
  ...
  if (education.length > 0) {
    ...
    for (const edu of education.slice(0, 3)) {
      ...
      if (edu.endDate) out += ` (${edu.endDate})`;
      out += `\n`;
    }
  }
...
// Tool 1: Search candidates by skills, location, or keyword
server.tool(
  "search_candidates",
  "Search the CVin.Bio professional database by skills, location, or keyword. Returns matching candidate profiles with work history, education, and skills.",
  {
    query: z
      .string()
      .describe(
        "Search term: a skill (e.g. 'React'), location (e.g. 'London'), job title (e.g. 'Product Manager'), or any keyword."
      ),
    limit: z
      .number()
      .min(1)
  ...
  async ({ query, limit }) => {
    ...
    const { data: profiles, error } = await supabase
      ...
      .or(
        `full_name.ilike.%${searchTerm}%,about.ilike.%${searchTerm}%,skills.cs.{${searchTerm}}`
      )
    ...
    if (error) {
      return {
        content: [
          { type: "text" as const, text: `Error searching: ${error.message}` },
        ],
      };
    }

    if (!profiles || profiles.length === 0) {
      // Fallback: broader search using experience/education JSONB
      const { data: fallback } = await supabase
        .from("profiles")
        .select("*")
        .not("username", "is", null)
        .or(
          `full_name.ilike.%${searchTerm}%,about.ilike.%${searchTerm}%`
        )
        .order("updated_at", { ascending: false })
        .limit(limit);
...
// Tool 2: Get a specific profile by username/slug
server.tool(
  "get_profile",
  "Get the full professional profile for a specific CVin.Bio user by their username (slug). Returns complete work history, education, skills, and custom sections.",
  {
    username: z
      .string()
      .describe(
        "The CVin.Bio username/slug (e.g. 'vedang'). This is the part after cvin.bio/ in the profile URL."
      ),
  },
  async ({ username }) => {
    ...
    for (const section of customSections) {
      ...
      for (const item of section.items || []) {
        extra += `- ${item.title}`;
        if (item.subtitle) extra += ` (${item.subtitle})`;
